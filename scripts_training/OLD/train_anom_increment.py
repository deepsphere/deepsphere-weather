...
def main(cfg_path, exp_dir, data_dir):
    ...
    scaler = SequentialScaler(
        LoadScaler(
            os.path.join(
                data_sampling_dir, "Scalers", "MonthlyStdAnomalyScaler_dynamic.nc"
            )
        ),
        LoadScaler(
            os.path.join(data_sampling_dir, "Scalers", "MonthlyStdAnomalyScaler_bc.nc")
        ),
        ...
        LoadScaler(
            os.path.join(
                data_sampling_dir,
                "Scalers",
                "MinMaxScaler_MonthlyStdAnomaly_dynamic.nc",
            )
        ),
        LoadScaler(
            os.path.join(
                data_sampling_dir, "Scalers", "MinMaxScaler_MonthlyStdAnomaly_bc.nc"
            )
        ),
        # Static data scaler
        LoadScaler(
    ...
    dict_DataArrays = reformat_Datasets(
    ...
    dict_test_DataArrays = reformat_Datasets(
        ds_training_dynamic=ds_test_dynamic,
        ds_static=ds_static,
    ...
    with open(
        os.path.join(exp_dir, "training_info/AR_TrainingInfo.pickle"), "wb"
    ) as handle:
