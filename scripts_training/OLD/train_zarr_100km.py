...
# cfg_path = './configs/UNetSpherical/Healpix_400km/MaxAreaPool-k20.json'
...
def main(cfg_path, exp_dir, data_dir):
    ...
    data_sampling_dir = (
        "/home/ghiggi/Projects/DeepSphere/data/preprocessed/ERA5_HRES/Healpix_100km"
    )

    da_dynamic = xr.open_zarr(
        os.path.join(
    ...
    da_static = xr.open_zarr(os.path.join(data_sampling_dir, "Data", "static.zarr"))[
        "data"
    ]

    da_bc = da_bc.sel(time=slice(None, "1980-03-31T23:00:00"))
    da_dynamic = da_dynamic.sel(feature=["z500", "t850"])
    model_settings["resolution"] = 64
    training_settings["training_batch_size"] = 2
    AR_settings["AR_iterations"] = 5
    # da_dynamic = ds_dynamic.drop(["level","lat","lon"])
    # da_bc = ds_bc.drop(["lat","lon"])
    ...
    static_scaler = LoadScaler(
        os.path.join(data_sampling_dir, "Scalers", "GlobalStandardScaler_static.nc")
    )
    ...
    check_AR_DataArrays(
        da_training_dynamic=da_test_dynamic,
    ...
    DeepSphereModelClass = getattr(
        my_architectures, model_settings["architecture_name"]
    )
    ...
    ###-----------------------------------------------------------------------.
    ### Summarize the model
    profiling_info = summarize_model(
        model=model,
    ...
    exp_dir = create_experiment_directories(
        exp_dir=exp_dir, model_name=model_name, force=True
    )
    ...
    training_info = AutoregressiveTraining(
        model=model,
    ...
    for AR_iteration in range(training_info.AR_iterations + 1):
        ax = training_info.plot_loss_per_AR_iteration(
    ...
    leg = ax.legend(
        labels=list(range(training_info.AR_iterations + 1)),
        title="AR iteration",
        loc="upper right",
    )
    ...
    # - Save figure
    fig.savefig(
        os.path.join(exp_dir, "figs/training_info/Loss_at_all_AR_iterations.png")
    )
    ##------------------------------------------------------------------------.
    ## - Plot the loss at each AR iteration (in separate figures)
    for AR_iteration in range(training_info.AR_iterations + 1):
        fname = os.path.join(
            exp_dir, "figs/training_info/Loss_at_AR_{}.png".format(AR_iteration)
        )
        training_info.plot_loss_per_AR_iteration(
            AR_iteration=AR_iteration,
            linewidth=0.6,
            ylim=(0, 0.06),
    ...
    ##------------------------------------------------------------------------.
    ## - Plot total loss
    training_info.plot_total_loss(ylim=(0, 0.06), linewidth=0.6).savefig(
        os.path.join(exp_dir, "figs/training_info/Total_Loss.png")
    )

    ##------------------------------------------------------------------------.
    ## - Plot AR weights
    training_info.plot_AR_weights(normalized=True).savefig(
        os.path.join(exp_dir, "figs/training_info/AR_Normalized_Weights.png")
    )
    training_info.plot_AR_weights(normalized=False).savefig(
        os.path.join(exp_dir, "figs/training_info/AR_Absolute_Weights.png")
    )
    ...
    forecast_zarr_fpath = os.path.join(
        exp_dir, "model_predictions/spatial_chunks/test_pred.zarr"
    )
    dask.config.set(scheduler="synchronous")
    ...
    verification_zarr_fpath = os.path.join(
        exp_dir, "model_predictions/temporal_chunks/test_pred.zarr"
    )
    ...
    # - Save sptial skills
    ds_skill.to_netcdf(
        os.path.join(exp_dir, "model_skills/deterministic_spatial_skill.nc")
    )
    ...
    ds_global_skill.to_netcdf(
        os.path.join(exp_dir, "model_skills/deterministic_global_skill.nc")
    )
    ...
    plot_global_skill(ds_global_skill).savefig(
        os.path.join(exp_dir, "figs/skills/RMSE_skill.png")
    )
    plot_global_skills(ds_global_skill).savefig(
        os.path.join(exp_dir, "figs/skills/skills_global.png")
    )
    plot_skills_distribution(ds_skill).savefig(
        os.path.join(exp_dir, "figs/skills/skills_distribution.png")
    )
    ...
    for month in [1, 4, 7, 10]:
        ...
        create_GIF_forecast_error(
            GIF_fpath=os.path.join(
                exp_dir, "figs/forecast_states", "M" + "{:02}".format(month) + ".gif"
            ),
    ...
    hourly_weekly_anomaly_scaler = LoadAnomaly(
        os.path.join(
            data_sampling_dir, "Scalers", "WeeklyHourlyStdAnomalyScaler_dynamic.nc"
        )
    )
    for month in [1, 4, 7, 10]:
        idx_month = np.argmax(
        ...
        ds_forecast = ds_forecasts.isel(forecast_reference_time=idx_month)
        create_GIF_forecast_anom_error(
            GIF_fpath=os.path.join(
                exp_dir, "figs/forecast_anom", "M" + "{:02}".format(month) + ".gif"
            ),
            ds_forecast=ds_forecast,
            ds_obs=ds_obs,
