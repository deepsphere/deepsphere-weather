...
# -----------------------------------------------------------------------------.
def main(data_dir, model_dirs, DeepEnsemble_dir, remove_model_dirs=False, force=False):
    ##------------------------------------------------------------------------.
    ### TODO:
    ...
    ##------------------------------------------------------------------------.
    # Define paths
    ens_forecast_fpath = os.path.join(
        DeepEnsemble_dir, f"model_predictions/temporal_chunks/test_pred_ens.zarr"
    )
    ens_median_forecast_fpath = os.path.join(
        DeepEnsemble_dir, f"model_predictions/temporal_chunks/test_pred_ens_median.zarr"
    )
    ##------------------------------------------------------------------------.
    # Get configs
    ...
    ens_members_fpaths = [
        os.path.join(model_dir, "model_predictions/temporal_chunks/test_pred.zarr")
    ...
    ds_obs_test, ds_ensemble_median = xr.align(
        ds_dynamic, ds_ensemble_median
    )  # TODO.. left side?
    ds_obs_test = ds_obs_test.chunk({"time": -1, "node": 1})
    ds_skill = xverif.deterministic(
        pred=ds_ensemble_median,
        obs=ds_obs_test,
    ...
    # - Save sptial skills
    ds_skill.to_netcdf(
        os.path.join(DeepEnsemble_dir, f"model_skills/deterministic_spatial_skill.nc")
    )
    ...
    ds_global_skill.to_netcdf(
        os.path.join(DeepEnsemble_dir, "model_skills/deterministic_global_skill.nc")
    )
    ...
    plot_global_skill(ds_global_skill).savefig(
        os.path.join(DeepEnsemble_dir, "figs/skills/RMSE_skill.png")
    )
    plot_global_skills(ds_global_skill).savefig(
        os.path.join(DeepEnsemble_dir, "figs/skills/skills_global.png")
    )
    plot_skills_distribution(ds_skill).savefig(
        os.path.join(DeepEnsemble_dir, "figs/skills/skills_distribution.png")
    )
    ...
    print(
        "- DeepEnsemble verification terminated. Elapsed time: {:.1f} hours ".format(
...
if __name__ == "__main__":
    ...
    default_model_dirs = glob.glob(
        "/data/weather_prediction/experiments/exp_DeepEnsemble/*"
    )
    default_model_dirs = " ".join(default_model_dirs)
    default_DeepEnsemble_dir = (
        "/data/weather_prediction/experiments/exp_DeepEnsemble/DeepEnsemble"
    )

    parser = argparse.ArgumentParser(
        description="Verify a DeepEnsemble of DeepSphere-Weather models"
    )
    parser.add_argument("--data_dir", type=str, default=default_data_dir)
    parser.add_argument(
        "--DeepEnsemble_dir", type=str, default=default_DeepEnsemble_dir
    )
