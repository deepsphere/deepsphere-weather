...
# -*- coding: utf-8 -*-
"""
Created on Sun Aug 22 16:57:29 2021
...
exp_dir = "/home/ghiggi/Projects/DeepSphere/data/experiments/exp_poolings"
...
pool_methods = ['Interp', 'MaxVal', 'MaxArea', 'Learn']
# Define graph type 
gtypes = ["knn",'mesh']
# Define model seed and data sets 
model_seed = 15    # Model seed is same across model (change only between n_models)
shuffle_seed = 25 # Provide same data to all samplings (change only between n_models)
#-----------------------------------------------------------------------------.
...
for i in range(1, n_models+1): 
    for sampling in samplings:
        for periodic_padding in periodic_paddings:
            # Define config file path      
            cfg_path = os.path.join(project_dir, "configs","UNetSpherical",
                                    sampling, pool_method + "Pool-Graph_"+ "knn" + ".json")
            # Read general config
            cfg = read_config_file(fpath=cfg_path)
            # Optional configs 
            cfg['training_settings']["deterministic_training"] = True
            ...
            if periodic_padding: 
                cfg['model_settings']["model_name_suffix"] = "PeriodicPadding_Model_ +" + str(i)  
            else:
                cfg['model_settings']["model_name_suffix"] = "Model_ +" + str(i)  
            cfg['model_settings']["model_name_prefix"] = model_name_prefix
