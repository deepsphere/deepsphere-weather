...
def main(cfg_path, exp_dir, data_dir, force=False):
    ...
    model_settings[
        "model_name_prefix"
    ] = "OLD_fine_tuned4_EncoderAllReZero2_FixedARWeight"
    ...
    if model_settings["pretrained_model_name"] is not None:
        ...
        load_pretrained_model(model=model, model_dir=pretrained_model_dir)
    ###-----------------------------------------------------------------------.
    ...
    write_config_file(cfg=cfg, fpath=os.path.join(model_dir, "config.json"))
    ...
    if training_settings["ar_training_strategy"] == "RNN":
        ar_scheduler = AR_Scheduler(
            method="LinearStep",
            factor=0.0005,
            fixed_ar_weights=np.arange(
                0, ar_settings["ar_iterations"]
            ),  # [0], TODO CHANGE
    ...
    ds_long_forecasts = AutoregressivePredictions(
        model=model,
        # Data
    ...
    for i in range(len(forecast_reference_times)):
        ...
        fig.savefig(
            os.path.join(
                model_dir, "figs/hovmoller_plots", "state_sim" + "{:01}.png".format(i)
            )
        )
        ...
        fig.savefig(
            os.path.join(
                model_dir, "figs/hovmoller_plots", "anom_sim" + "{:01}.png".format(i)
            )
        )
...
if __name__ == "__main__":
    default_data_dir = (
        "/ltenas3/data/DeepSphere/data/preprocessed/ERA5_HRES"  # new data
    )
