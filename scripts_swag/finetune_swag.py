...
def main(cfg_path, exp_dir, data_dir, train=True, pred=True):
    ...
    cfg["model_settings"][
        "model_name"
    ] = "RNN-UNetSpherical-icosahedral-16-k20-MaxAreaPooling-float32-AR6-LinearStep-SWAG-LR0007"
    ...
    dim_info = get_ar_model_tensor_info(
    ...
    # - Define SWAG model
    swag_model = get_pytorch_SWAG_model(
        module=my_architectures,
    ...
    if model_settings["pretrained_model_name"] is not None:
        ar_scheduler = load_pretrained_ar_scheduler(
            exp_dir=experiments_dir, model_name=model_settings["pretrained_model_name"]
        )
    else:
    ...
    if pred:
        ...
        forecast_zarr_fpath = os.path.join(
            exp_dir,
            f"model_predictions/spatial_chunks/test_pred_{sampling_scale}_median.zarr",
        )
        dask.config.set(scheduler="synchronous")
        ds_forecasts = AutoregressiveSWAGPredictions(
            model=swag_model,
            exp_dir=exp_dir,
            # Data
            training_data_dynamic=da_training_dynamic,
            ...
            # Dataloader options
            device=device,
            batch_size=training_settings[
                "training_batch_size"
            ],  # number of forecasts per batch
    ...
    verification_zarr_fpath = os.path.join(
        exp_dir, f"model_predictions/temporal_chunks/test_pred_{sampling_scale}.zarr"
    )
    ...
    ds_skill.to_netcdf(
        os.path.join(
            exp_dir, f"model_skills/deterministic_spatial_skill_{sampling_scale}.nc"
        )
    )
    ...
    ds_global_skill.to_netcdf(
        os.path.join(
            exp_dir, f"model_skills/deterministic_global_skill_{sampling_scale}.nc"
        )
    )
    ...
    plot_skill_maps(
        ...
        figs_dir=os.path.join(
            exp_dir, f"figs/sampling_{sampling_scale}/skills/SpatialSkill"
        ),
    ...
    plot_global_skill(ds_global_skill).savefig(
        os.path.join(exp_dir, f"figs/sampling_{sampling_scale}/skills/RMSE_skill.png")
    )
    plot_global_skills(ds_global_skill).savefig(
        os.path.join(
            exp_dir, f"figs/sampling_{sampling_scale}/skills/skills_global.png"
        )
    )
    plot_skills_distribution(ds_skill).savefig(
        os.path.join(
            exp_dir, f"figs/sampling_{sampling_scale}/skills/skills_distribution.png"
        )
    )
    ...
    for month in range(1, 13):
        ...
        create_gif_forecast_error(
            gif_fpath=os.path.join(
                exp_dir,
                f"figs/sampling_{sampling_scale}/forecast_states",
                "M" + "{:02}".format(month) + ".gif",
            ),
    ...
    for month in range(1, 13):
        ...
        create_gif_forecast_anom_error(
            gif_fpath=os.path.join(
                exp_dir,
                f"figs/sampling_{sampling_scale}/forecast_anom",
                "M" + "{:02}".format(month) + ".gif",
            ),
            ds_forecast=ds_forecast,
            ds_obs=ds_obs,
