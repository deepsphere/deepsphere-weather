...
for resolution in resolutions:
    ...
    graph = get_pygsp_graph(sampling=sampling, resolution=resolution, knn=knn)
    ...
    for knn in knn_list:
        ...
        optimizer = torch.optim.Adam(
            model.parameters(), lr=0.008, eps=1e-7, weight_decay=0, amsgrad=False
        )
        ##---------------------------------------------------------------------.
        l_forward_time = []
        l_backward_time = []
...
for knn in knn_list:
    ...
    RGG_grids_labels = [
        "O" + str(int(x / 2)) for x in list(forward_timing_dict[knn].keys())
    ]
