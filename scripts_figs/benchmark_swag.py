...
def main(base_dir, figs_dir, benchmark_dir):
    ...
    ##-----------------------------------------------------------------------------.
    # Load Weyn benchmark 
    weyn_skills = xr.open_dataset("/mnt/scratch/students/haddad/weather_prediction/data/healpix/metrics/rmses_weyn.nc")
    ...
    benchmark_global_skill(skills_dict=skills_dict, 
                        skill="RMSE", 
                        variables=['z500','t850'],
                        n_leadtimes=20).savefig(os.path.join(figs_dir, "Benchmark_Models_SWAG_RMSE.png"))
