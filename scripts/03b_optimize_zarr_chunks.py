...
raw_pl_dirpath = os.path.join(
    data_dir, "raw", "ERA5_HRES", sampling, "dynamic", "pressure_levels"
)
...
for time_chunk in time_chunks:
    chunks = {"node": -1, "time": time_chunk, "feature": 1}
    ds = ds.chunk(chunks)
...
# Select time_chunks
chunks = {"node": -1, "time": 24 * 365 * 1, "feature": 1}
...
for cname, compressor in compressors.items():
    ...
    timing_dict[cname] = profile_zarr_io(
        ds=ds,
...
compression_ratios = [
    timing_dict[cname]["compression_ratio"] for cname in selected_cnames
]
