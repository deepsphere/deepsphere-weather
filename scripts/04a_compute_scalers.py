...
import warnings
import xarray as xr
from xscaler import GlobalStandardScaler, GlobalMinMaxScaler, AnomalyScaler, Climatology
...
def compute_scalers(
    sampling_name, base_data_dir, reference_period, DataArrayFormat=False, force=True
):
    ##------------------------------------------------------------------------.
    ...
    dynamic_scaler.fit()
    dynamic_scaler.save(
        os.path.join(data_dir, "Scalers", "GlobalStandardScaler_dynamic.nc"),
        force=force,
    )
    del dynamic_scaler
    client.run(gc.collect)
    ...
    bc_scaler.fit()
    bc_scaler.save(
        os.path.join(data_dir, "Scalers", "GlobalStandardScaler_bc.nc"), force=force
    )
    del bc_scaler
    client.run(gc.collect)
    ...
    dynamic_scaler.fit()
    dynamic_scaler.save(
        os.path.join(data_dir, "Scalers", "GlobalMinMaxScaler_dynamic.nc"), force=force
    )
    del dynamic_scaler
    client.run(gc.collect)
    ...
    bc_scaler.fit()
    bc_scaler.save(
        os.path.join(data_dir, "Scalers", "GlobalMinMaxScaler_bc.nc"), force=force
    )
    del bc_scaler
    client.run(gc.collect)
    ...
    dynamic_scaler.fit()
    dynamic_scaler.save(
        os.path.join(data_dir, "Scalers", "MonthlyStdAnomalyScaler_dynamic.nc"),
        force=force,
    )

    bc_scaler = AnomalyScaler(
        data_bc,
        time_dim="time",
        time_groups="month",
        variable_dim=variable_dim,
    ...
    bc_scaler.fit()
    bc_scaler.save(
        os.path.join(data_dir, "Scalers", "MonthlyStdAnomalyScaler_bc.nc"), force=force
    )
    ...
    dynamic_minmax_scaler = GlobalMinMaxScaler(
        data=data_dynamic_anom, variable_dim=variable_dim
    )
    dynamic_minmax_scaler.fit()
    dynamic_minmax_scaler.save(
        os.path.join(data_dir, "Scalers", "MinMaxScaler_MonthlyStdAnomaly_dynamic.nc"),
        force=force,
    )
    del dynamic_scaler
    ...
    bc_minmax_scaler.save(
        os.path.join(data_dir, "Scalers", "MinMaxScaler_MonthlyStdAnomaly_bc.nc"),
        force=force,
    )
    ...
    dynamic_scaler.fit()
    dynamic_scaler.save(
        os.path.join(data_dir, "Scalers", "WeeklyStdAnomalyScaler_dynamic.nc"),
        force=force,
    )
    ...
    bc_scaler.fit()
    bc_scaler.save(
        os.path.join(data_dir, "Scalers", "WeeklyStdAnomalyScaler_bc.nc"), force=force
    )
    ...
    monthly_clim.save(
        os.path.join(data_dir, "Climatology", "MonthlyClimatology_dynamic.nc"),
        force=force,
    )
    del monthly_clim
    client.run(gc.collect)
    ...
    weekly_clim.compute()
    weekly_clim.save(
        os.path.join(data_dir, "Climatology", "WeeklyClimatology_dynamic.nc"),
        force=force,
    )
    del weekly_clim
    client.run(gc.collect)
    ...
    daily_clim.compute()
    daily_clim.save(
        os.path.join(data_dir, "Climatology", "DailyClimatology_dynamic.nc"),
        force=force,
    )
    del daily_clim
    client.run(gc.collect)
    ...
    hourlymonthly_clim = Climatology(
        data=data_dynamic,
        time_dim="time",
    ...
    hourlymonthly_clim.save(
        os.path.join(data_dir, "Climatology", "HourlyMonthlyClimatology_dynamic.nc"),
        force=force,
    )
