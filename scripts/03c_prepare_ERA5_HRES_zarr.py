...
def zarrify_raw_data(
    ...
    toa_zarr_fpath = os.path.join(
        zarr_dataset_dirpath, sampling, "Data", "bc", dst_subdirname, "bc.zarr"
    )
    ...
    for fpath in toa_fpaths:
        ...
        if len(ds["time"]) > 0:
            ...
            ds = reformat_toa(
                ds=ds, var_dict=toa_var_dict, stack_variables=stack_variables
            )
            # - Write data to zarr
            write_zarr(
...
# -----------------------------------------------------------------------------.
def rechunk_zarr_stores(
    zarr_dataset_dirpath, sampling, chunks, src_subdirname, dst_subdirname
):
    print("==================================================================")
    ...
    toa_timechunked_zarr_fpath = os.path.join(
        zarr_dataset_dirpath, sampling, "Data", "bc", src_subdirname, "bc.zarr"
    )
...
stack_variables = (
    False  # --> If True the output is a DataArray with 'feature' dimension
)
start_time = "1980-01-01T07:00:00"
end_time = "2018-12-31T23:00:00"

# - Define chunking options for the various samplings
chunks_400km = {"node": -1, "time": 24 * 7}
chunks_100km = {"node": -1, "time": 24 * 2}
