...
Y_dynamic_torch = training_batch_dict['Y'][0][sample,...,feature_idx].numpy().squeeze()

x_feature = data_dynamic.to_array('feature').isel(feature=slice(feature_idx,feature_idx+1)).sel(time=slice(time_Y, time_Y+1)).load()
x_feature = x_feature.transpose(..., 'feature')
x_std_feature = scaler.transform(x_feature, variable_dim="feature").compute().values.squeeze()
