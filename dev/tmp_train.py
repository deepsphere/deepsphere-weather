...
forecast_zarr_fpath = os.path.join(
    exp_dir, "model_predictions/forecast_chunked/test_forecasts.zarr"
)
dask.config.set(
...
verification_zarr_fpath = os.path.join(
    exp_dir, "model_predictions/space_chunked/test_forecasts.zarr"
)
ds_verification_format = rechunk_forecasts_for_verification(
    ds=ds_forecasts, chunks="auto", target_store=verification_zarr_fpath, max_mem="2GB"
)
...
# - Compute skills
ds_obs = xr.open_zarr(
    os.path.join(data_sampling_dir, "Data", "dynamic", "space_chunked", "dynamic.zarr")
)
