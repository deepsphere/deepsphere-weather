...
class ConvBlock(GeneralConvBlock):
    ...
    def __init__(
        ...
        if batch_norm:
            bias = False
        # Define convolution
        self.conv = GeneralConvBlock.getConvLayer(
...
class UNetSpherical(UNet, torch.nn.Module):
    ...
    def __init__(
        ...
        for i in range(1, UNet_depth):
            ...
            sampling_kwargs_list.append(
                pygsp_graph_coarsening(
                    sampling=sampling,
        ...
        self.conv13 = ConvBlock(
            32 * 2, 64 * 2, laplacian=self.laplacians[0], **convblock_kwargs
        )
        ...
        self.conv21 = ConvBlock(
            64 * 2, 96 * 2, laplacian=self.laplacians[1], **convblock_kwargs
        )

        self.conv23 = ConvBlock(
            96 * 2, 128 * 2, laplacian=self.laplacians[1], **convblock_kwargs
        )

        self.conv2_res = Linear(64 * 2, 128 * 2)

        # Encoding block 3
        self.conv31 = ConvBlock(
            128 * 2, 256 * 2, laplacian=self.laplacians[2], **convblock_kwargs
        )

        self.conv33 = ConvBlock(
            256 * 2, 128 * 2, laplacian=self.laplacians[2], **convblock_kwargs
        )
        ...
        self.uconv21 = ConvBlock(
            256 * 2, 128 * 2, laplacian=self.laplacians[1], **convblock_kwargs
        )
        self.uconv22 = ConvBlock(
            128 * 2, 64 * 2, laplacian=self.laplacians[1], **convblock_kwargs
        )

        # Decoding block 1
        self.uconv11 = ConvBlock(
            128 * 2, 64 * 2, laplacian=self.laplacians[0], **convblock_kwargs
        )
        self.uconv12 = ConvBlock(
            64 * 2, 32 * 2, laplacian=self.laplacians[0], **convblock_kwargs
        )
