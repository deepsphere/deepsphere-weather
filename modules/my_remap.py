...
def get_variable_interp_method_dict():
    ...
    d = {
        "dynamic_variables": "conservative",
...
def get_cdo_grid_fpath(CDO_grids_dir, sampling):
    ...
    if not os.path.exists(fpath):
        raise ValueError(
            "Please create a CDO grid description infile into the CDO grids folder"
        )
    return fpath
...
def get_src_dst_fpaths(src_dir, dst_dir):
    ...
    dst_fpaths = [
        re.sub(pattern=src_dir, repl=dst_dir, string=fpath) for fpath in src_fpaths
    ]
...
def remap_grib_files(
    ...
    src_CDO_grid_fpath = get_cdo_grid_fpath(
        CDO_grids_dir=CDO_grids_dir, sampling=native_sampling
    )
    dst_CDO_grid_fpath = get_cdo_grid_fpath(
        CDO_grids_dir=CDO_grids_dir, sampling=sampling
    )
    ##------------------------------------------------------------------------.
    if variable_type == "static":
    ...
    for variable in variables:
        ...
        src_fpaths, dst_fpaths = get_src_dst_fpaths(src_dir=src_dir, dst_dir=dst_dir)
        ##--------------------------------------------------------------------.
        if len(src_fpaths) == 0:
        ...
        if force_remapping is not True:
            idx_not_existing = [
                not os.path.exists(dst_fpath) for dst_fpath in dst_fpaths
            ]
        ...
        if len(src_fpaths) == 0:
            print(
                "Data were already remapped. Set force_remapping=True to force remapping."
            )
            continue
        ##--------------------------------------------------------------------.
        ...
        cdo_weights_name = get_cdo_weights_filename(
            method=method, input_sampling=native_sampling, output_sampling=sampling
        )
