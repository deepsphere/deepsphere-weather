...
class DeepSphere(ABC):
    ...
    @staticmethod
    def build_pygsp_graphs(
        sampling_list: List[str], sampling_kwargs_list: List[Dict]
    ) -> List["pygsp.graphs"]:
        """Build the graphs for the specified samplings."""
        # Checks
        if not isinstance(sampling_list, list):
            raise TypeError(
                "sampling_list must be a list specifying the sampling of each graph."
            )
        if not isinstance(sampling_list, list):
            raise TypeError(
                "sampling_kwargs_list must be a list specifying the sampling_kwargs_list of each sampling."
            )
        if len(sampling_list) != len(sampling_kwargs_list):
            raise ValueError(
                "sampling_list must have same length of sampling_kwargs_list."
            )
        # Create a list of pygsp graphs
        pygsp_graphs_list = [
            get_pygsp_graph_fun(sampl)(**sampl_kwargs, lap_type="normalized")
            for sampl, sampl_kwargs in zip(sampling_list, sampling_kwargs_list)
        ]
        return pygsp_graphs_list

    @staticmethod
    def get_laplacian_kernels(graphs: List["pygsp.graphs"], graph_type: str = "knn"):
        """Compute the laplacian for each specified graph."""
        assert graph_type in ["knn", "voronoi"]
        laplacians_list = [
            graph.L
            if graph_type == "knn"
            else compute_cotan_laplacian(graph, return_mass=False)
            for graph in graphs
        ]
        laplacians_list = [prepare_torch_laplacian(L) for L in laplacians_list]
        return laplacians_list

    def init_graph_and_laplacians(
        self,
        ...
        if conv_type == "graph":
            self.graphs = DeepSphere.build_pygsp_graphs(
                sampling_list=sampling_list, sampling_kwargs_list=sampling_kwargs_list
            )

            self.laplacians = DeepSphere.get_laplacian_kernels(
                graphs=self.graphs, graph_type=graph_type
            )
