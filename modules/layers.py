...
def _import_igl():
    try:
        ...
        raise ImportError(
            "Cannot import igl. Build a knn graph "
...
def scale_operator(laplacian, lmax, scale=1):
    ...
    sparse_I = sparse.identity(
        laplacian.shape[0], format=laplacian.format, dtype=laplacian.dtype
    )
...
def conv_cheb(laplacian, inputs, weight):
    ...
    if not Fin1 == Fin:
        raise ValueError(
            "Input tensor shape does not match the expected shape: \n"
            + "- Input tensor shape :{} \n".format(Fin1)
...
class ConvCheb(torch.nn.Module):
    ...
    def reset_parameters(self, activation="relu", fan="in", distribution="normal"):
...
def get_nlat_nlon(n_nodes, lonlat_ratio):
    ...
    assert (
        n_lat * n_lon == n_nodes
    ), f"Unable to unpack nodes: {n_nodes}, lonlat_ratio: {lonlat_ratio}"
    return n_lat, n_lon
...
def _build_interpolation_matrix(src_graph, dst_graph):
    ...
    ds = compute_interpolation_weights(
        src_graph=src_graph,
        dst_graph=dst_graph,
        method="conservative",
...
def convert_to_torch_sparse(mat: "sparse.coo.coo_matrix"):
    ...
    mat = torch.sparse_coo_tensor(
        indices, mat.data, mat.shape, dtype=torch_dtype, device=indices.device
    )
    mat = mat.coalesce()
    return mat
    ...
    def forward(self, x):
        ...
        if self.return_indices:
            x, indices = F.max_pool2d(
                x, self.kernel_size, return_indices=self.return_indices
            )
        else:
...
class EquiangularAvgUnpool(torch.nn.Module):
    ...
    def forward(self, x, *args):
        ...
        x = x.permute(0, 3, 1, 2)
        x = F.interpolate(
            x, scale_factor=(self.kernel_size, self.kernel_size), mode="nearest"
        )
        x = x.permute(0, 2, 3, 1)
        x = equiangular_2d_to_1d(x)
        return x
...
class HealpixAvgUnpool(torch.nn.Module):
    ...
    def forward(self, x, *args):
        ...
        x = torch.nn.functional.interpolate(
            x, scale_factor=self.kernel_size, mode="nearest"
        )
...
class GeneralMaxValUnpool(RemapBlock):
    ...
    def forward(self, x, index, *args, **kwargs):
        ...
        x_unpooled = torch.zeros(
            [new_nodes, n_batch * n_val], dtype=x.dtype, device=x.device
        )
        row, col = index
...
# ####################################
HEALPIX_POOL = {
    "max": (HealpixMaxPool, HealpixMaxUnpool),
    "avg": (HealpixAvgPool, HealpixAvgUnpool),
}

EQUIANGULAR_POOl = {
    "max": (EquiangularMaxPool, EquiangularMaxUnpool),
    "avg": (EquiangularAvgPool, EquiangularAvgUnpool),
}

ALL_POOL = {"healpix": HEALPIX_POOL, "equiangular": EQUIANGULAR_POOl}
...
def get_conv_fun(conv_type):
    ...
    ALL_CONV = {"image": Conv2dEquiangular, "graph": ConvCheb}
    ...
    def getConvLayer(
        ...
        **kwargs,
        ...
        if conv_type == "graph":
            ...
            raise ValueError(
                "{} conv_type is not supported. Choose either 'graph' or 'image'".format(
                    conv_type
                )
            )
        return conv
